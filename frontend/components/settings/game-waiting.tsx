...
import {   ClarityValue,
  TupleCV,
  ClarityType, } from "@stacks/transactions";
...
type ClarityLike =
  ...
  | { type: "false" }
  | { type: "none" }
  | { type: "some"; value: ClarityLike };

function cvToJs(cv: ClarityLike): unknown {
  switch (cv.type) {
    case "uint":
    case "int":
      return cv.value;
...
function tupleToObject(tuple: TupleCV) {
  const obj: Record<string, any> = {};

  for (const [key, cv] of Object.entries(tuple.value)) {
    const camelKey = key.replace(/-([a-z])/g, (_, c) => c.toUpperCase());
    obj[camelKey] = cvToJs(cv as any);
  }
...
function isTupleCV(cv: ClarityValue): cv is TupleCV {
...
useEffect(() => {
  ...
  const run = async () => {
    const contractData: ClarityValue =
      await handleGetGameByCode(gameCode);

    if (!isTupleCV(contractData)) {
      console.error("Expected TupleCV, got:", contractData);
      return;
    }

  
    const gameObject = tupleToObject(contractData) as Game;
setContractGame(gameObject);
console.log("contract game object", gameObject);
...
return (
  <div>
    {/* <p>Type: {contractGame?.type}</p>
    <p>Value: {contractGame?.value}</p> */}
  </div>
);
